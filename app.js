...
function rules(){
  ...
  <div class="grid rules-grid">
    <article class="card rule"><h3>Active career model</h3><div class="formula">30% current performance + 25% achievements + 20% potential + 15% audience + 10% availability</div><p>Metrics must come from career evidence. Roster membership alone receives a conservative provisional price capped below star-level valuations.</p><div class="weight-list">${Object.entries(taxonomy.pricingModels.active.weights).map(([k,v])=>`<div class="weight-row"><span>${esc(k)}</span><strong>${Math.round(v*100)}%</strong></div>`).join('')}</div></article>
