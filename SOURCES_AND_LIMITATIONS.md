# Sources, Attribution, and Limitations

## Fjelstul World Cup Database

Player records are derived from the Fjelstul World Cup Database v1.2.0.

Required attribution: **Joshua C. Fjelstul, Ph.D., © 2023**. Licensed under **Creative Commons Attribution-ShareAlike 4.0 International (CC BY-SA 4.0)**. This prototype modifies the source by selecting, ranking, normalizing, and combining records with simulated market fields.

Project repository: https://github.com/jfjelstul/worldcup
License: https://creativecommons.org/licenses/by-sa/4.0/

## Pantheon 1.0

Historical biography records are derived from Pantheon 1.0. Citation requested by the publisher:

Yu, A. Z. et al. (2016), *Pantheon 1.0, a manually verified dataset of globally famous biographies*, Scientific Data 2:150075.

Dataset page: https://pantheon.world/data

## TMDB 5000 sample

Actor and selected music-credit records are derived from a historical TMDB 5000 credits sample used for prototyping. This package does not represent the sample as a current, complete, or commercially licensed talent feed. Original source terms, attribution requirements, TMDB API terms, and any redistribution permissions must be reviewed before commercial use.

## Curated anchor records

A small number of familiar public figures are included as manually curated prototype anchors. Their basic labels are not guaranteed current. Production records require authoritative identity resolution, current source checks, and correction workflows.

## Simulated fields

The following are generated for interface and model testing and are not real market or career valuations:

- Share price and fundamental value
- Daily percentage movement
- Price history and charts
- Market volume
- Career factor scores
- Demand premium and momentum
- Ranking and valuation range

## Product and legal limitations

Before public or commercial deployment, obtain qualified legal review and implement at least:

- Current licensed data feeds and source-specific commercial permissions
- Publicity-rights, trademark, endorsement, and false-association review
- Identity correction, removal, and dispute workflows
- Explainable and auditable price-event records
- Fraud, manipulation, and bot protections
- Age controls and responsible-use protections
- State and federal gaming, securities, commodities, sweepstakes, and payments analysis for any prizes or real money

This prototype does not provide ownership, royalties, revenue participation, or any legal interest in a person's career.
