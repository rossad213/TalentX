...
(function(){
  ...
  window.confirmTradeTicket=function(){
    if(!pendingTrade)return;
    const record=byId(pendingTrade.id),input=document.querySelector('#ticketShares');if(!record||!input)return;
    const q=quote(record,pendingTrade.mode,input.value),owned=Number(state.holdings[record.id]||0),cashBefore=state.cash;
    if(pendingTrade.mode==='buy'&&q.total>state.cash){toast('Not enough virtual cash');return;}
    if(pendingTrade.mode==='sell'&&q.n>owned){toast('You do not own that many shares');return;}
    ...
    state.transactions.unshift({id:record.id,mode:pendingTrade.mode,shares:q.n,price:q.execution,total:q.total,time:Date.now(),cashBefore,cashAfter:state.cash});
    state.transactions=state.transactions.slice(0,200);saveState();
    const completed={record,mode:pendingTrade.mode,q};closeTradeLayer();showTradeConfirmation(completed);render();
  };
