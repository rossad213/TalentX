...
def merge_market_state(
    ...
    prior_by_id = {
        ...
        if record.get("id") and (record.get("lastPriceEventId") or record.get("priceHistory") or record.get("priceEvents"))
    }
    ...
    for record in rebuilt:
        ...
        if prior and isinstance(prior_price, (int, float)) and float(prior_price) > 0:
            ...
            # A rebuild refreshes fundamentals but does not manufacture another
            # observable market move. Existing verified events stay durable.
