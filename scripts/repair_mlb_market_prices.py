...
* clears live game-move fields that no longer describe the restored price;
* removes corrupted game events from the live event list while preserving draft,
  signing, team-change, and other non-game events;
* prunes obviously corrupt chart observations and appends the restored price;
...
REPAIR_VERSION = "1.0-mlb-v1-market-recovery"
...
def game_event(event: Any) -> bool:
    ...
    event_key = str(event.get("eventKey") or event.get("eventId") or "").strip().lower()
    provider = str(event.get("provider") or "").strip().lower()
    return event_type == "game" or event_key.startswith("espn:") or (provider == "espn" and event_type != "career")
...
def safe_history(record: dict[str, Any], anchor: float, repaired_at: str) -> list[dict[str, Any]]:
    ...
    for point in history:
        ...
        if price is None:
            continue
        # Preserve plausible verified history but drop the runaway observations
...
def repair_record(record: dict[str, Any], *, repaired_at: str) -> tuple[dict[str, Any], bool]:
    ...
    result["previousMarketPrice"] = round(old_price, 2) if old_price is not None else anchor
    result["marketPrice"] = anchor
    result["dailyChange"] = 0.0
    result["hourlyChangePct"] = 0.0
    result["lastGameMovePct"] = 0.0
    result["trend"] = [anchor] * 18
    result["priceEvents"] = preserved_events
    result["priceHistory"] = safe_history(result, anchor, repaired_at)
