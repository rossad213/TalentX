...
(function(){
  ...
  function eligibility(range){
    ...
    return {trades,total,eligible:trades>=2&&total>=MIN_PORTFOLIO_VALUE};
  }
  function rowsFor(range){
    ...
    if(consented()&&status.eligible){
      ...
      rows.push({name:displayName(),total:status.total,returnPct:((status.total-capital)/capital*100)*factor,trades:status.trades,score:Math.min(.95,.5+Math.min(20,status.trades)*.015),you:true});
    }
  ...
  window.joinTalentXLeaderboard=function(){
    ...
    if(!status.eligible){toast('Complete two trades and maintain at least $200 before joining');return;}
  ...
  window.leaderboard=function(){
    ...
    return `${note()}<div class="eyebrow">Community competition</div><h1 class="page-title">Leaderboard</h1><p class="page-sub">Participation is optional. Purchased TX Cash is treated as contributed capital and never as investment profit.</p>
    ...
    <section class="card p4-rules"><strong>Privacy and fair ranking</strong><span>Users are private and unranked by default.</span><span>Joining requires explicit permission and a chosen public display name.</span><span>Purchased TX Cash does not count as profit.</span><span>You can leave the leaderboard at any time.</span><span>Ranked by percentage return with at least 2 trades and $200 portfolio value.</span></section>
